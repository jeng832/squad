...
- 세션 시작 시 `squad-{sessionId}-{agentId}` 규칙으로 Agent 컨테이너를 생성한다.
- 시작 전에 동일 `sessionId`의 잔여 컨테이너가 있으면 먼저 정리한다.
- 세션 실행 중 Tool 호출은 해당 `sessionId/agentId` 컨테이너가 `running`일 때만 허용한다.
- Tool 계층은 컨테이너를 자동 재시작하지 않는다. 비실행 상태는 즉시 실패 처리한다.
- 세션 취소/완료 시 해당 `sessionId` 컨테이너를 모두 종료/삭제한다.
- Health check는 세션 컨테이너 자동 복구를 수행하지 않고 상태 관측/로그만 수행한다.
